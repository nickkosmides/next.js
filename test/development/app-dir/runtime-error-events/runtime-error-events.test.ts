...
async function observe(url: string) {
  ...
  return { messages, close: () => socket.terminate() }
}
...
describe('runtime-error-events', () => {
  ...
  it('republishes current state after client navigation', async () => {
    ...
    try {
      ...
      // Compiling the destination may refresh and clear the overlay. The
      // contract is its current state at the new pathname, not error history.
      await retry(() => {
        expect(observer.messages).toContainEqual(
          expect.objectContaining({
