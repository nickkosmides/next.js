...
const BASE_VARIANTS = [
  {
    suffix: 'baseline',
    imports: `import { installNextJs, installPlaywrightSystemDependencies, prepareFixture } from '../lib/setup.js'`,
    setup: `await installNextJs(sandbox)\n    await prepareFixture(sandbox)\n    await installPlaywrightSystemDependencies(sandbox)`,
  },
  {
    suffix: 'agents-md',
    imports: `import { installNextJs, installPlaywrightSystemDependencies, prepareFixture, writeAgentsMd } from '../lib/setup.js'`,
...
function getExperimentSettings(evalName) {
  ...
  const skillVariants = [...skillGroups.values()].map(({ skills, evals }) => ({
    ...
    imports: `import { installLocalSkills, installNextJs, installPlaywrightSystemDependencies, prepareFixture } from '../lib/setup.js'`,
